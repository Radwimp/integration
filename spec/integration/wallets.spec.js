describe("Wallet", () => {
  ...
  describe("Currencies", () => {
    ...
    it('should have deposit address', function () {
      cy.request('http://www.streamex.tech/api/v2/peatio/public/currencies')
      .then(response => {
        cy.wrap(response.body).each((T) => {
          if (T.type === 'coin') {
            cy.request(`http://www.streamex.tech/api/v2/peatio/account/deposit_address/${T.id}`)
            .then(response => {
              cy.wait(500);
                cy.get('.cr-wallet-list').contains(T.name).click();
                cy.get('#copy_deposit_1').then($input => {
                  expect($input.val()).to.be.a('string');
                  expect($input.val().length).to.equal(response.body.address.length);
              });
            });
          }
        });
